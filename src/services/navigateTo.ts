...
namespace ts.NavigateTo {
    ...
    function getItemsFromNamedDeclaration(patternMatcher: PatternMatcher, name: string, declarations: ReadonlyArray<Declaration>, checker: TypeChecker, fileName: string, rawItems: Push<RawNavigateToItem>): void {
        ...
        const match = patternMatcher.getMatchesForLastSegmentOfPattern(name); //name
        if (!match) {
            return; // continue to next named declarations
        }
        const { kind, isCaseSensitive } = match;
        ...
        for (const declaration of declarations) {
            ...
            if (patternMatcher.patternContainsDots) {
                ...
                //&& (!patternMatcher.patternContainsDots || patternMatcher.getFullMatch(getContainers(declaration), name))) {
                rawItems.push({ name, fileName, matchKind: kind, isCaseSensitive, declaration });
            }
        }
    }
